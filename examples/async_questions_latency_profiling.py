...
@P.semantic_function(formatter=P.allowing_newline)
def Passage5(question:P.Input, response: P.Output):
    """Role: Pretend you are a high school student answering questions for an english class. Read the passage below and answer the subsequent question.
...
@P.semantic_function(formatter=P.allowing_newline)
def Passage6(question:P.Input, response: P.Output):
    """Role: Pretend you are a high school student answering questions for an english class. Read the passage below and answer the subsequent question.
...
@P.semantic_function(formatter=P.allowing_newline)
def Passage7(question:P.Input, response: P.Output):
    """Role: Pretend you are a high school student answering questions for an english class. Read the passage below and answer the subsequent question.
...
@P.semantic_function(formatter=P.allowing_newline)
def Passage8(question:P.Input, response: P.Output):
    """Role: Pretend you are a high school student answering questions for an english class. Read the passage below and answer the subsequent question.
...
async def do():
    global end
    prefix_choices = [Passage1, Passage2, Passage3, Passage4, Passage5, Passage6, Passage7, Passage8]
    question_bank = []
    with open('question_bank.txt', 'r') as f:
        for line in f:
            question_bank.append(line.strip())
    nums = [x % len(question_bank) for x in range(NUM_SAMPLES)]
    calls = []
    ...
    for x in range(NUM_SAMPLES):
        def thing(ind):
            ...
            prefix = prefix_choices[ind % 8]
            question = question_bank[nums[ind]]#random.choice(question_bank)
            response = prefix(question).get()
            print("Returned")
            time_end = time.time()
            return (question, response, ind, time_end - time_start, time_start)
        calls.append(loop.run_in_executor(executor, thing, x))
    done, pending = await asyncio.wait(calls, return_when=asyncio.ALL_COMPLETED)
    
    results = [task.result() for task in done]
    results = sorted(results, key=lambda x: x[-1])
    plt.hist([val[3] for val in results], bins=20, color='blue', edgecolor='black')
    plt.savefig("histogram.png", dpi=300, bbox_inches="tight") 

    print(pd.Series([x[3] for x in results]).describe())
    # loop = asyncio.get_event_loop()
    #results = loop.run_until_complete(asyncio.gather(*calls))
    end = time.time()
    for x in results:
        print("Index", x[2], "Time diff:", x[3], "Time start:", time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(x[4])))
        # print("----------------------------------------", x)
...
def main():
    ...
    print(NUM_SAMPLES/elapsed_time, "requests / sec")
