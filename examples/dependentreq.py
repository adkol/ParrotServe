...
@P.semantic_function()
def say(question: P.Output):
    """You at the grocery store and you see some bananas and you are hungry
    ask your bro to figure out how many bananas you want
    Your question: {{question}}"""

@P.semantic_function()
def respond(question: P.Input, response: P.Output):
    """Your bro asked you this please respond harshly
    User input: {{question}}
    Your output: {{response}}"""

# Then we can start to define the main function.
def main():
    say_out = say()  # print by the semantic code
    respond_out = respond(say_out)
    print("Question:", say_out.get())
    print("Response:", respond_out.get())

    print(respond_out.get(P.PerformanceCriteria.LATENCY))  # print by the native code
