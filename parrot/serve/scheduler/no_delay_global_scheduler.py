...
class GlobalScheduler:
    ...
    def _get_engine_list(
        ...
        def check_engine_available(engine: ExecutionEngine):
            ...
            print("ENGINE NUM TASKS", engine.get_num_tasks(), engine.get_tasks_num_upperbound(), engine.get_remain_tasks_capacity(), engine.get_remain_tokens_capacity())
            # print(len(tasks))
            if 1 + engine.get_num_tasks() > 6:#tasks_num_upperbound // 2:
                return False
            # Check whether it violates the tasks_num_upperbound of the engine.
            if len(tasks) + engine.get_num_tasks() > 6:#engine.get_tasks_num_upperbound() // 2:
                return False
                

            # Check whether the engine has enough task capacity.
            if len(tasks) > engine.get_remain_tasks_capacity() // 2:
                return False
            ...
            if model_type == ModelType.TOKEN_ID:
                ...
                if total_tokens_num > engine.get_remain_tokens_capacity() // 2:
                    return False
    ...
    def _find_engine(self, tasks: List[CompletionTask]) -> None:
        ...
        print("SCHEDULING ON ENGINE")
        print(best_engine, best_engine.get_num_tasks())
        print(len(engine_list))
        for i, task in enumerate(tasks):
            print("scheduled one", i)
